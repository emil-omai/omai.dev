---
title: "Om oss"
url: "/om-oss/"
aliases: ["/about/"]
---

## Vi är OMAI

<div class="team-grid">
{{< team-member src="images/about/kajsa.grip.webp" alt="Kajsa Grip, CEO OMAI" >}}
### Kajsa Grip — Grundare & CEO
Kajsa brinner för att sänka bruset och hitta den mänskliga nyttan i tekniken. Med lång erfarenhet inom innovation och ny teknik ser hon till att AI-resan blir begriplig, trygg och strategiskt rätt för just er verksamhet.  
Kontakt: [kajsa.grip@omai.dev](mailto:kajsa.grip@omai.dev)
{{< /team-member >}}

{{< team-member src="images/about/emil.breding.webp" alt="Emil Breding, CTO OMAI" >}}
### Emil Breding — Grundare & CTO 
Emil är experten som får tekniken att flytta in i verkligheten. Genom att kombinera teknisk spets med ett öga för praktiska lösningar ser han till att de verktyg ni implementerar faktiskt skapar mätbar affärsnytta och ett enklare arbetsliv.   
Kontakt: [emil.breding@omai.dev](mailto:emil.breding@omai.dev)
{{< /team-member >}}
</div>

## Därför väljer företag OMAI

{{< highlight >}}
För att vi ser både tekniken och människan bakom den.
{{< /highlight >}}

Vi tror på människocentrerade AI – intelligenta lösningar som byggs på insikt, empati och affärsvärde. Vi hjälper dig att hitta rätt balans mellan innovation och verklighet, mellan möjligheter och ansvar. Oavsett var du står på din AI-resa möter vi dig där, och tar nästa steg tillsammans.

{{< features-section title="Nyckelvärden" >}}
{{< feature-card image="images/man-heart.png" iconColor="coral" title="Mänskligt" >}}
Vi börjar med förståelse, inte kod.
{{< /feature-card >}}

{{< feature-card image="images/star-hand-drawn.png" iconColor="blue" title="Strategiskt" >}}
Vi kopplar tekniken till din affär.
{{< /feature-card >}}

{{< feature-card image="images/2-faces.png" iconColor="yellow" title="Tillsammans" >}}
Vi bygger kunskap – och ger dig verktygen.
{{< /feature-card >}}
{{< /features-section >}}

